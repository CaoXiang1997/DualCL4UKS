...
class BertConfig(object):
  ...
  def from_json_file(cls, json_file):
    ...
    with torch.io.gfile.GFile(json_file, "r") as reader:
      text = reader.read()
...
class BertModel(nn.Module):
  ...
  input_word_ids = torch.constant([[31, 51, 99], [15, 5, 0]])
  input_mask = torch.constant([[1, 1, 1], [1, 1, 0]])
  input_type_ids = torch.constant([[0, 0, 1], [0, 2, 0]])
  ...
  def __init__(self, config, float_type=torch.float32, **kwargs):
    ...
    self.encoder = Transformer(
        ...
        name="encoder")
    self.pooler_transform = nn.Linear(
  ...
  def forward(self, inputs, mode="bert"):
    """Implements forward() for the layer.
    ...
    if self.float_type == torch.float16:
      embedding_tensor = torch.cast(embedding_tensor, torch.float16)
    attention_mask = None
    ...
    first_token_tensor = torch.squeeze(sequence_output[:, 0:1, :], dim=1)
...
class EmbeddingLookup(nn.Module):
  ...
  def forward(self, inputs):
    ...
    flat_input = torch.reshape(inputs, [-1])
    output = torch.gather(self.embeddings, flat_input)
    output = torch.reshape(output, input_shape + [self.embedding_size])
    return output
...
class EmbeddingPostprocessor(nn.Module):
  ...
  def __init__(self,
    ...
    self.output_layer_norm = nn.LayerNorm(
        name="layer_norm", dim=-1, epsilon=1e-12)
    self.output_dropout = torch.keras.layers.Dropout(rate=self.dropout_prob)
  ...
  def forward(self, inputs):
    ...
    if self.use_type_embeddings:
      flat_token_type_ids = torch.reshape(token_type_ids, [-1])
      one_hot_ids = torch.one_hot(
          flat_token_type_ids,
          depth=self.token_type_vocab_size,
          dtype=self.dtype)
      token_type_embeddings = torch.matmul(one_hot_ids, self.type_embeddings)
      token_type_embeddings = torch.reshape(token_type_embeddings,
                                         [batch_size, seq_length, width])
      output += token_type_embeddings

    if self.use_position_embeddings:
      position_embeddings = torch.unsqueeze(
          torch.slice(self.position_embeddings, [0, 0], [seq_length, width]),
          dim=0)
...
class Attention(nn.Module):
  ...
  In practice, the multi-headed attention are done with torch.einsum as follows:
    Input_tensor: [BFD]
    Wq, Wk, Wv: [DNH]
  ...
  def __init__(self,
    ...
    self.attention_probs_dropout = torch.keras.layers.Dropout(
  ...
  def forward(self, inputs):
    ...
    # attention scores.
    attention_scores = torch.einsum("BTNH,BFNH->BNFT", key_tensor, query_tensor)
    ...
    if attention_mask is not None:
      ...
      adder = (1.0 - torch.cast(attention_mask, attention_scores.dtype)) * -10000.0
    ...
    attention_probs = torch.softmax(attention_scores)
    ...
    context_tensor = torch.einsum("BNFT,BTNH->BFNH", attention_probs, value_tensor)
...
class Dense3D(nn.Module):
  """A Dense Layer using 3D kernel with torch.einsum implementation."""
  ...
  def __init__(self,
    ...
    self.backward_compatible = backward_compatible
    dtype = torch.as_dtype(self.dtype or torch.keras.backend.floatx())
    if not (dtype.is_floating or dtype.is_complex):
      raise TypeError("Unable to build `Dense` layer with non-floating point "
                      "dtype %s" % (dtype,))
    input_shape = torch.TensorShape(input_shape)
    if torch.compat.dimension_value(input_shape[-1]) is None:
      raise ValueError("The last dimension of the inputs to `Dense` "
                       "should be defined. Found `None`.")
    self.last_dim = torch.compat.dimension_value(input_shape[-1])
    self.input_spec = torch.keras.layers.InputSpec(
  ...
  def forward(self, inputs):
    """Implements ``forward()`` for Dense3D.
    ...
    """
    if self.backward_compatible:
      kernel = torch.keras.backend.reshape(self.kernel, self.kernel_shape)
      bias = torch.keras.backend.reshape(self.bias, self.bias_shape)
    else:
      kernel = self.kernel
      bias = self.bias

    if self.output_projection:
      ret = torch.einsum("abcd,cde->abe", inputs, kernel)
    else:
      ret = torch.einsum("abc,cde->abde", inputs, kernel)
    ret += bias
...
class Dense2DProjection(nn.Module):
  """A 2D projection layer with torch.einsum implementation."""
  ...
  def __init__(self,
    ...
    if torch.compat.dimension_value(input_shape[-1]) is None:
      raise ValueError("The last dimension of the inputs to `Dense` "
                       "should be defined. Found `None`.")
    last_dim = torch.compat.dimension_value(input_shape[-1])
    self.input_spec = torch.keras.layers.InputSpec(min_ndim=3, axes={-1: last_dim})
    self.kernel = self.add_weight(
        "kernel",
  ...
  def forward(self, inputs):
    """Implements forward() for Dense2DProjection.
    ...
    Returns:
      A 3D Tensor.
    """
    ret = torch.einsum("abc,cd->abd", inputs, self.kernel)
    ret += self.bias
    if self.activation is not None:
      return self.activation(ret)
    return ret
...
class TransformerBlock(nn.Module):
  ...
  def __init__(self,
    ...
    self.attention_dropout = torch.keras.layers.Dropout(
        rate=self.hidden_dropout_prob)
    self.attention_layer_norm = (
        nn.LayerNorm(
            name="self_attention_layer_norm", dim=-1, epsilon=1e-12))
    ...
    self.output_dense = Dense2DProjection(
        ...
        kernel_initializer=get_initializer(self.initializer_range),
        name="output")
    self.output_dropout = torch.keras.layers.Dropout(rate=self.hidden_dropout_prob)
    self.output_layer_norm = nn.LayerNorm(
        name="output_layer_norm", dim=-1, epsilon=1e-12)
  ...
  def forward(self, inputs):
    ...
    if self.float_type == torch.float16:
      input_tensor = torch.cast(input_tensor, torch.float32)
      attention_output = torch.cast(attention_output, torch.float32)
    attention_output = self.attention_layer_norm(input_tensor +
                                                 attention_output)
    intermediate_output = self.intermediate_dense(attention_output)
    if self.float_type == torch.float16:
      intermediate_output = torch.cast(intermediate_output, torch.float16)
    layer_output = self.output_dense(intermediate_output)
    layer_output = self.output_dropout(layer_output)
    # Use float32 in keras layer norm for numeric stability
    if self.float_type == torch.float16:
      layer_output = torch.cast(layer_output, torch.float32)
    layer_output = self.output_layer_norm(layer_output + attention_output)
    if self.float_type == torch.float16:
      layer_output = torch.cast(layer_output, torch.float16)
    return layer_output
...
def pack_inputs(inputs):
  ...
  Returns:
    ...
    tensor.
  """
  inputs = torch.nest.flatten(inputs)
  outputs = []
  for x in inputs:
    if x is None:
      outputs.append(torch.constant(0, shape=[], dtype=torch.int32))
    else:
      outputs.append(x)
...
def is_special_none_tensor(tensor):
  ...
  return tensor.shape.ndims == 0 and tensor.dtype == torch.int32
...
def get_activation(identifier):
  """Maps a string to a Python function, e.g., "relu" => `torch.nn.relu`.
...
def get_initializer(initializer_range=0.02):
  """Creates a `torch.initializers.truncated_normal` with the given range.
  ...
  """
  return torch.keras.initializers.TruncatedNormal(stddev=initializer_range)
...
def get_shape_list(tensor, expected_rank=None, name=None):
  ...
  Args:
    tensor: A torch.Tensor object to find the shape of.
...
def assert_rank(tensor, expected_rank, name=None):
  ...
  Args:
    tensor: A torch.Tensor to check the rank of.
...
def create_attention_mask_from_input_mask(from_tensor, to_mask):
  ...
  to_mask = torch.cast(
      torch.reshape(to_mask, [batch_size, 1, to_seq_length]),

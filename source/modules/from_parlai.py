...
def universal_sentence_embedding(sentences, sentences_length):
    ...
    :param Tensor sentences: an N x T x D of Transformer outputs. Note this is
        the exact output of TransformerEncoder, but has the time dim first
    :param ByteTensor: an N x T binary matrix of paddings

    :return: an N x D matrix of sentence embeddings
    :rtype Tensor:
    """
    sentences_sum = torch.sum(sentences, dim=1)
    divisor = torch.unsqueeze(torch.sqrt(sentences_length.to(torch.float32)), dim=-1)
    sentences_sum = sentences_sum / (divisor + 1e-7)

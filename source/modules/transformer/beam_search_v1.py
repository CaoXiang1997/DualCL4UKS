...
class SequenceBeamSearch(object):
  ...
  def search(self, initial_ids, initial_cache):
    ...
    finished_state = torch.while_loop(
    ...
    finished_seq = torch.where(
        torch.reduce_any(finished_flags, 1), finished_seq, alive_seq)
    finished_scores = torch.where(
        torch.reduce_any(finished_flags, 1), finished_scores, alive_log_probs)
  ...
  def _create_initial_state(self, initial_ids, initial_cache):
    ...
    initial_log_probs = torch.constant(
        [[0.] + [-float("inf")] * (self.beam_size - 1)])
    alive_log_probs = torch.tile(initial_log_probs, [self.batch_size, 1])
    ...
    finished_seq = torch.zeros(torch.shape(alive_seq), torch.int32)

    # Set scores of the initial finished seqs to negative infinity.
    finished_scores = torch.ones([self.batch_size, self.beam_size]) * -INF

    # Initialize finished flags with all False values.
    finished_flags = torch.zeros([self.batch_size, self.beam_size], torch.bool)
    ...
    state_shape_invariants = {
        _StateKeys.CUR_INDEX: torch.TensorShape([]),
        _StateKeys.ALIVE_SEQ: torch.TensorShape([None, self.beam_size, None]),
        _StateKeys.ALIVE_LOG_PROBS: torch.TensorShape([None, self.beam_size]),
        _StateKeys.ALIVE_CACHE: nest.map_structure(
            _get_shape_keep_last_dim, alive_cache),
        _StateKeys.FINISHED_SEQ: torch.TensorShape([None, self.beam_size, None]),
        _StateKeys.FINISHED_SCORES: torch.TensorShape([None, self.beam_size]),
        _StateKeys.FINISHED_FLAGS: torch.TensorShape([None, self.beam_size])
    }
  ...
  def _continue_search(self, state):
    ...
    not_at_max_decode_length = torch.less(i, self.max_decode_length)
    ...
    finished_scores *= torch.cast(finished_flags,
                               torch.float32)  # set filler scores to zero
    lowest_finished_scores = torch.reduce_min(finished_scores, dim=1)

    # If there are no finished sequences in a batch element, then set the lowest
    # finished score to -INF for that element.
    finished_batches = torch.reduce_any(finished_flags, 1)
    lowest_finished_scores += (1.0 -
                               torch.cast(finished_batches, torch.float32)) * -INF

    worst_finished_score_better_than_best_alive_score = torch.reduce_all(
        torch.greater(lowest_finished_scores, best_alive_scores)
    )

    return torch.logical_and(
        not_at_max_decode_length,
        torch.logical_not(worst_finished_score_better_than_best_alive_score)
    )
  ...
  def _grow_alive_seq(self, state):
    ...
    topk_log_probs, topk_indices = torch.nn.top_k(flat_log_probs, k=beams_to_keep)
    ...
    topk_ids = torch.unsqueeze(topk_ids, dim=2)
    topk_seq = torch.concat([topk_seq, topk_ids], dim=2)
  ...
  def _get_new_alive_state(self, new_seq, new_log_probs, new_cache):
    ...
    new_finished_flags = torch.equal(new_seq[:, :, -1], self.eos_id)
    new_log_probs += torch.cast(new_finished_flags, torch.float32) * -INF
  ...
  def _get_new_finished_state(self, state, new_seq, new_log_probs):
    ...
    finished_seq = torch.concat(
        [finished_seq,
         torch.zeros([self.batch_size, self.beam_size, 1], torch.int32)], dim=2)
    ...
    new_finished_flags = torch.equal(new_seq[:, :, -1], self.eos_id)
    new_scores += (1. - torch.cast(new_finished_flags, torch.float32)) * -INF

    # Combine sequences, scores, and flags.
    finished_seq = torch.concat([finished_seq, new_seq], dim=1)
    finished_scores = torch.concat([finished_scores, new_scores], dim=1)
    finished_flags = torch.concat([finished_flags, new_finished_flags], dim=1)
...
def sequence_beam_search(
  ...
  """
  batch_size = torch.shape(initial_ids)[0]
...
def _log_prob_from_logits(logits):
  return logits - torch.reduce_logsumexp(logits, dim=2, keepdims=True)


def _length_normalization(alpha, length):
  """Return length normalization factor."""
  return torch.pow(((5. + torch.cast(length, torch.float32)) / 6.), alpha)
...
def _expand_to_beam_size(tensor, beam_size):
  ...
  return torch.tile(tensor, tile_dims)
...
def _shape_list(tensor):
  ...
  dynamic_shape = torch.shape(tensor)
...
def _get_shape_keep_last_dim(tensor):
  ...
  if isinstance(shape_list[-1], torch.Tensor):
    shape_list[-1] = None
  return torch.TensorShape(shape_list)
...
def _gather_beams(nested, beam_indices, batch_size, new_beam_size):
  ...
  batch_pos = torch.reshape(batch_pos, [batch_size, new_beam_size])

  # Create coordinates to be passed to torch.gather_nd. Stacking creates a tensor
  ...
  return nest.map_structure(
      lambda state: torch.gather_nd(state, coordinates), nested)


def _gather_topk_beams(nested, score_or_log_prob, batch_size, beam_size):
  """Gather top beams from nested structure."""
  _, topk_indexes = torch.nn.top_k(score_or_log_prob, k=beam_size)

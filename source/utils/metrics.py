...
def cosine(X, Y):
    ...
    cosine
    """
    sim = np.sum(X * Y, dim=1) / \
        (np.sqrt((np.sum(X * X, dim=1) * np.sum(Y * Y, dim=1))) + 1e-10)
    return sim
...
class EmbeddingMetrics(object):
    ...
    def average(self, embeds):
        ...
        average
        """
        avg_embeds = [embed.mean(dim=0) for embed in embeds]
    ...
    def extrema(self, embeds):
        ...
        ext_embeds = []
        for embed in embeds:
            s_max = np.max(embed, dim=0)
            s_min = np.min(embed, dim=0)
    ...
    def greedy(self, hyp_embeds, ref_embeds):
        ...
        for hyp_embed, ref_embed in zip(hyp_embeds, ref_embeds):
            ...
            g_sim = (cos_sim.max(dim=1).mean() +
                     cos_sim.max(dim=0).mean()) / 2

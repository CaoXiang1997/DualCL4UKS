...
class KnowledgeSeq2Seq(BaseModel):
    ...
    def encode(self, inputs, training=False, use_posterior=True):
        """
        encode
        """
        outputs = Pack()
    ...
    def collect_metrics(self, outputs, target, epoch=-1, use_posterior=True):
        """
        collect_metrics
        """
        ...
        if self.use_posterior and use_posterior:
            if self.posterior_detach:
                outputs.posterior_attn = outputs.posterior_attn.detach()
            
            if self.loss_form == "kl_loss":
                kl_loss = outputs.posterior_attn * (torch.log(outputs.posterior_attn + 1e-10) - torch.log(outputs.prior_attn + 1e-10))
            elif self.loss_form == "js_loss":
                avg_p = (outputs.posterior_attn + outputs.prior_attn) / 2
                kl_loss = outputs.posterior_attn * (torch.log(outputs.posterior_attn + 1e-10) - torch.log(avg_p + 1e-10)) / 2
                kl_loss += outputs.prior_attn * (torch.log(outputs.prior_attn + 1e-10) - torch.log(avg_p + 1e-10)) / 2
            elif self.loss_form == "dual_kl_loss":
                kl_loss = outputs.posterior_attn * (torch.log(outputs.posterior_attn + 1e-10) - torch.log(outputs.prior_attn + 1e-10)) / 2
                kl_loss += outputs.prior_attn * (torch.log(outputs.prior_attn + 1e-10) - torch.log(outputs.posterior_attn + 1e-10)) / 2
    ...
    def forward(self, inputs, training=False, epoch=-1, use_posterior=True):
        ...
        metrics = self.collect_metrics(outputs, target, epoch=epoch, use_posterior=use_posterior)

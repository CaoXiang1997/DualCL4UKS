...
def tokenize(s):
    ...
    """
    s = s.lower()
    s = re.sub("(["+string.punctuation+"])", ' \\1', s)
...
class Field(object):
    ...
    def numericalize(self, strings, bos_and_eos=True):
        """
        numericalize
        """
        if self.tokenizer is not None:
            ...
            if isinstance(strings, str):
                return self.str2num(strings, bos_and_eos)
            else:
                return [self.numericalize(s, bos_and_eos) for s in strings]
    ...
    def denumericalize(self, numbers):
        ...
        if self.tokenizer is not None:
            if isinstance(numbers[0], list):
                ...
                return self.tokenizer.decode(numbers, skip_special_tokens=True)
        else:
            if self.sequential:
...
class TextField(Field):
    ...
    def build_word_embeddings(self, embed_file):
        ...
        elif isinstance(embed_file, str):
                    ...
                    for line in f:
                        ...
                        if w in self.stoi:
                            ...
                            if len(vs) == dim:
                                ...
                                cover += 1
            elif embed_file.endswith('pt'):
                _, stoi, embeddings, dim = torch.load(embed_file)
                embeds = [[0] * dim] * len(self.stoi)
                for w,_ in self.stoi.items():
                    if w in stoi:
                        embeds[self.stoi[w]] = embeddings[stoi[w]].tolist()
                        cover += 1
            else:
                raise Exception("'{}' is unresolvable data type...".format(embed_file))
    ...
    def str2num(self, string, bos_and_eos=True):
        """
        str2num
        """
        tokens = []
        ...
        if self.bos_token and bos_and_eos:
            tokens.append(self.bos_token)

        tokens += self.tokenize_fn(string)

        if self.eos_token and bos_and_eos:
